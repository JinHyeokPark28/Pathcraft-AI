...
def parse_pob_xml(xml_string, pob_url):
    ...
    try:
        ...
        final_guide = {
            "meta": {"build_name": build_name, "class": class_name, "ascendancy": asc_name, "pob_link": pob_url, "version": build.get('targetVersion')},
            "build_notes": build_notes,
            "overview": {"summary": "", "pros": [], "cons": []}, "leveling": {"summary": "", "early_skills": [], "vendor_regex": {}},
            "progression_stages": [{"stage_name": "Final Build", "pob_link": pob_url, "passive_tree_url": passive_tree_url, "ascendancy_order": [], "gem_setups": gem_setups, "gear_recommendation": gear, "bandit": build.get('bandit'), "pantheon": {"major": build.get('pantheonMajorGod'), "minor": build.get('pantheonMinorGod')}}]
        }
        print("   > 성공: POB 데이터를 최종 가이드 형식으로 변환했습니다.")
        return final_guide
    except Exception as e:
