...
**가격:**
- 구독: $2.50/월 (Stripe/PayPal)
- 후원: Patreon/Ko-fi (1회성 또는 정기)

**수익 모델:**
- 광고 수익: $200-500/월 (예상, DAU 500+ 기준)
- 구독 수익: $125/월 (50명 × $2.50)
- 후원 수익: $50-100/월

**전환 전략:**
- 3일 무료 체험 → 광고 없는 경험
- 광고가 성가신 사용자 → 프리미엄 구독
- 월 $2.50 = POE Overlay와 동일 (익숙한 가격)
...
**Goal:** POE Overlay 대체 + AI 아이템 추천 (차별화)

**Tasks:**
- [ ] **기본 오버레이**
  - [ ] POE 게임 프로세스 감지
  - [ ] 투명 윈도우 오버레이 (Always-on-top)
  - [ ] 단축키 토글 (F10 등)
  - [ ] Ctrl+C 아이템 복사 감지
...
- [ ] **AI 아이템 추천 (🔥 차별화 킬러 기능!)**
  - [ ] 현재 캐릭터 정보 읽기 (POE API)
  - [ ] POB 빌드 연동
  - [ ] "이 아이템 지금 쓸까요?" AI 분석
    - 레벨 단계 확인 (레벨 28 → Tabula 추천)
    - 저항 부족 감지 (Cold 30% 부족 → 반지 추천)
    - 예산 고려 (50c 소지 → 100c 아이템 대안 제시)
    - DPS/생존력 개선도 예측
  - [ ] 대안 아이템 추천
  - [ ] 빌드 진행도 추적 (시작 → 중간 → 완성)
...
- [ ] **빌드 가이드 오버레이**
  - [ ] 레벨링 체크리스트 (F10 토글)
  - [ ] 다음 스킬 변경 알림 (Level 28: Caustic Arrow)
  - [ ] 다음 업그레이드 아이템 표시

**Deliverables:**
- POE Overlay 기능 대체 (가격 체크)
- AI 아이템 추천 (독보적 차별화!)
- 레벨링 가이드 오버레이

**경쟁 우위:**
| 기능 | POE Overlay | PathcraftAI Overlay |
|------|-------------|---------------------|
| 가격 표시 | ✅ | ✅ |
| 아이템 복사 | ✅ | ✅ |
| **AI "지금 쓸까?" 분석** | ❌ | ✅ 🔥 |
